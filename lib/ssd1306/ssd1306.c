...
// Send data
void ssd1306_write_data(uint8_t *buffer, size_t size)
{
  // Select OLED
  gpio_clear(SSD1306_BANK_CS, SSD1306_CS);
  // Data
...
// Screenbuffer
uint8_t ssd1306_screen_buffer[SSD1306_BUFFER_SIZE] = {0};
...
void ssd1306_fill(SSD1306_Color color)
{
  ...
  for (i = 0; i < sizeof(ssd1306_screen_buffer); i++)
  {
...
void ssd1306_update(void)
{
  ...
  for (uint8_t i = 0; i < SSD1306_HEIGHT / 8; i++)
  {
    ...
    ssd1306_write_data(&ssd1306_screen_buffer[SSD1306_WIDTH * i], SSD1306_WIDTH);
  }
}
...
void ssd1306_draw_pixel(uint8_t x, uint8_t y, SSD1306_Color color)
{
  ...
  if (color == White)
  {
    ssd1306_screen_buffer[x + (y / 8) * SSD1306_WIDTH] |= 1 << (y % 8);
  }
  else
  {
    ssd1306_screen_buffer[x + (y / 8) * SSD1306_WIDTH] &= ~(1 << (y % 8));
...
{
  return (ssd1306_screen_buffer[x + (y / 8) * SSD1306_WIDTH] & (1 << (y % 8))) ? White : Black;
}
...
static float ssd1306_deg_to_rad(float par_deg)
{
  return par_deg * 3.14 / 180.0;
}

/* Normalize degree to [0;360] */
static uint16_t ssd1306_normalize_to0_360(uint16_t par_deg)
{
  uint16_t loc_angle;
  if (par_deg <= 360)
  {
...
void ssd1306_draw_arc(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, uint16_t sweep, SSD1306_Color color)
{
  ...
  loc_sweep = ssd1306_normalize_to0_360(sweep);

  count = (ssd1306_normalize_to0_360(start_angle) * CIRCLE_APPROXIMATION_SEGMENTS) / 360;
  approx_segments = (loc_sweep * CIRCLE_APPROXIMATION_SEGMENTS) / 360;
  approx_degree = loc_sweep / (float)approx_segments;
  while (count < approx_segments)
  {
    rad = ssd1306_deg_to_rad(count * approx_degree);
    ...
    {
      rad = ssd1306_deg_to_rad(loc_sweep);
    }
...
void ssd1306_draw_circle(uint8_t par_x, uint8_t par_y, uint8_t par_r, SSD1306_Color par_color)
{
  int32_t x = -par_r;
  int32_t y = 0;
...
void ssd1306_fill_circle(uint8_t par_x, uint8_t par_y, uint8_t par_r, SSD1306_Color par_color)
{
  ...
  do
  {
    for (uint8_t _y = (par_y + y); _y >= (par_y - y); _y--)
    {
      ...
      {
        ssd1306_draw_pixel(_x, _y, par_color);
      }
    }
