 ...
 * the display data RAM under Page Addressing Mode. The column address will be
 * incremented by each data access. Please refer to Section Table 9-1 and
 ...
 * be incremented by each data access. Please refer to Section Table 9-1 and
 * Section 10.1.3 for details.
 */
 ...
 * content. The sequence of movement of the PAGE and column address point for
 * page addressing mode is shown in Figure 10-1. In normal display data RAM read
 * or write and page addressing mode, the following steps are required to define
 * the starting RAM access pointer location: • Set the page start address of the
 * target display location by command B0h to B7h. • Set the lower start column
 * address of pointer by command 00h~0Fh. • Set the upper start column address
 * of pointer by command 10h~1Fh. For example, if the page address is set to
 * B2h, lower column address is 03h and upper column address is 10h, then that
 * means the starting column is SEG3 of PAGE2. The RAM access pointer is located
 * as shown in Figure 10-2. The input data byte will be written into RAM
 ...
 * column address pointer is increased automatically by 1. If the column address
 * pointer reaches column end address, the column address pointer is reset to
 * column start address and page address pointer is increased by 1. The sequence
 * of movement of the page and column address point for horizontal addressing
 * mode is shown in Figure 10-3. When both column and page address pointers
 * reach the end address, the pointers are reset to column start address and
 * page start address (Dotted line in Figure 10-3.)
 */
 ...
 * reaches the page end address, the page address pointer is reset to page start
 * address and column address pointer is increased by 1. The sequence of
 * movement of the page and column address point for vertical addressing mode is
 * shown in Figure 10-4. When both column and page address pointers reach the
 * end address, the pointers are reset to column start address and page start
 * address (Dotted line in Figure 10-4.) In normal display data RAM read or
 * write and horizontal / vertical addressing mode, the following steps are
 * required to define the RAM access pointer location:
 * • Set the column start and end address of the target display location by
 * command 21h. • Set the page start and end address of the target display
 * location by command 22h. Example is shown in Figure 10-5.
 */
 ...
 * column address pointer finishes accessing the end column address, it is reset
 * back to start column address and the row address is incremented to the next
 * row.
 */
 ...
 * incremented automatically to the next page address. Whenever the page address
 * pointer finishes accessing the end page address, it is reset back to start
 * page address.
 * The figure below shows the way of column and page address pointer movement
 * through the example: column start address is set to 2 and column end address
 * is set to 125, page start address is set to 1 and page end address is set to
 * 6; Horizontal address increment mode is enabled by command 20h. In this case,
 * the graphic display data RAM column accessible range is from column 2 to
 * column 125 and from page 1 to page 6 only. In addition, the column address
 * pointer is set to 2 and page address pointer is set to 1. After finishing
 * read/write one pixel of data, the column address is increased automatically
 * by 1 to access the next RAM location for next read/write operation (solid
 * line in Figure 10-5). Whenever the column address pointer finishes accessing
 * the end column 125, it is reset back to column 2 and page address is
 * automatically increased by 1 (solid line in Figure 10-5). While the end page
 * 6 and end column 125 RAM location is accessed, the page address is reset back
 * to 1 and the column address is reset back to 2 (dotted line in Figure 10-5).
 */
 ...
 * to 0, RAM row 0 is mapped to COM0. With value equal to 1, RAM row 1 is mapped
 * to COM0 and so on. Refer to Table 10-1 for more illustrations.
 */
 ...
 * the segment driver. It allows flexibility in OLED module design. Please refer
 * to Table 9-1. This command only affects subsequent data input. Data already
 ...
 * command will be turned ON. When the display is OFF, those circuits will be
 * turned OFF and the segment and common output are in VSS state and high
 * impedance state, respectively. These commands set the display to one of the
 * two states: o AEh : Display OFF o AFh : Display ON
 */
 ...
 * Page Addressing Mode. Please refer to Table 9-1 and Section 10.1.3 for
 * details.
 */
 ...
 * immediately. Please refer to Table 10-3 for details.
 */
 ...
 * display start line then the display start line register is equal to 0). For
 * example, to move the COM16 towards the COM0 direction by 16 lines the 6-bit
 * data in the second byte should be given as 010000b. To move in the opposite
 * direction by 16 lines the 6-bit data should be given by 64 – 16, so the
 * second byte would be 100000b. The following two tables (Table 10-1, Table
 * 10-2) show the example of setting the command C0h/C8h and D3h.
 */
 ...
 * Set the divide ratio to generate DCLK (Display Clock) from CLK. The divide
 * ratio is from 1 to 16, with reset value = 1. Please refer to section 8.3 for
 * the details relationship of DCLK and CLK. • Oscillator Frequency (A[7:4])
 * Program the oscillator frequency Fosc that is the source of CLK if CLS pin is
 * pulled high. The 4-bit value results in 16 different frequency settings
 * available as shown below. The default setting is 1000b.
 */
 ...
 * hardware layout. The table below shows the COM pin configuration under
 * different conditions (for MUX ratio =64):
 */
